 ...
 *  	2011-08-30: Add option '-a' to output [A]ll the information of methyl-C sites;
 */
...
void exit_with_help( void )
{
	printf(
		...
		"Author:	Guo, Weilong	guoweilong@gmail.com 	2011-08-30\n"
		"Options:\n"
...
//		"	binary searching is used by default\n"
		...
		"-a	(optional switch) Output all the information of mC\n"
...
int OutputSite (int rpos, unsigned int left, unsigned int right, Site& site) {
	// rpos = relative position
	if ( param.methyl ) {
		if ( param.debug ) {
			cout << rpos << "\t" << site.chr << "\t" << site.pos << "\t[" << left << ",\t" << right << "]\t" << site.strand << "\t" << site.MC << "\t" << site.ALLC << endl; 
		} else {
			cout << rpos << "\t" << site.MC << "\t" << site.ALLC << endl; 
		}
	} else if ( param.all ) {
		cout << rpos << "\t" << site.chr << "\t" << site.pos << "\t" << site.strand << "\t" << site.pattern << "\t" << site.MC << "\t" << site.ALLC << endl; 
	} else {
		if ( param.debug ) {
			cout << rpos << "\t" << site.chr << "\t" << site.pos << "\t[" << left << ",\t" << right << "]\t" << site.strand << endl; 
		} else {
			cout << rpos << endl; 
		}
	}
...
int MapSiteFile (map<string, vector<Region> > & Genome) {
	...
	while (!Sfile.eof()) {
		...
		site.pattern = tokens[3];
		site.MC = atoi(tokens[4].c_str());
		site.ALLC = atoi(tokens[5].c_str());
		...
		if (site.pos >= left && site.pos <= right) {
			if (strand == '+') {
				OutputSite (site.pos - left, left, right, site) ;
			} else {
				OutputSite (right - site.pos, left, right, site) ;
			}
		}
		int p = mid-1;
		while (p >= 0 && site.pos >= Genome[chr][p].left && site.pos <= Genome[chr][p].right) {
			if (Genome[chr][p].strand == '+') {
				OutputSite (site.pos - Genome[chr][p].left, Genome[chr][p].left, Genome[chr][p].right, site);
			} else {
				OutputSite (Genome[chr][p].right - site.pos, Genome[chr][p].left, Genome[chr][p].right, site);
			}
			p--;
		}
		p = mid+1;
		while (p < size && site.pos >= Genome[chr][p].left && site.pos <= Genome[chr][p].right) {
			if (Genome[chr][p].strand == '+') {
				OutputSite (site.pos - Genome[chr][p].left, Genome[chr][p].left, Genome[chr][p].right, site);
			} else {
				OutputSite (Genome[chr][p].right - site.pos, Genome[chr][p].left, Genome[chr][p].right, site);
