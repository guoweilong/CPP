/* BiasMethyl.cpp
 * Modification:
 * 2011-10-27, if strand specific methylation is zero?
 * 	Add '-I'. specific region of Intron?
 */
...
{
	printf(
		"Usage:	BiasMethyl -s <site.bed> -r <refFlat> [-I <region_of_Intron>]\n"
		...
		"-I	5SSR1 5SSR2 MI 3SSR2 3SSR1 whole\n"
		"	[5,25][26,125][126,-126][-125,-26][-25,-5][5,-5]\n"
		);
	exit(1);
}
...
int RegionBiasedMethylation ( vector<Site> & Chr, unsigned int IntronLeft, unsigned int IntronRight,
...
int IntronBiasedMethylation ( map< string, vector<Site> > & Genome, list<RefFlat> & Annotation ) {
	list<RefFlat>::iterator liter;
	for ( liter = Annotation.begin(); liter != Annotation.end(); liter++ ) {
		string chr= liter->chr;
		char strand = liter->strand;
		unsigned int n = liter->nExon;
		list<Site> WStrand, CStrand;
		for ( int i = 1; i < n; i++ ) {
			...
			if ( param.IR == "whole" ) {
				...
				if (IntronRight > IntronLeft + 25)
					RegionBiasedMethylation ( Genome[chr], IntronLeft+5, IntronLeft+25, WStrand, CStrand);
			} else if ( param.IR == "5SSR2" ) {
				if (IntronRight > IntronLeft + 125)
					RegionBiasedMethylation ( Genome[chr], IntronLeft+26, IntronLeft+125, WStrand, CStrand);
			} else if ( param.IR == "MI" ) {
				if (IntronRight > IntronLeft + 250)
					RegionBiasedMethylation ( Genome[chr], IntronLeft+126, IntronRight-126, WStrand, CStrand);
			} else if ( param.IR == "3SSR2" ) {
				if (IntronRight > IntronLeft + 125)
					RegionBiasedMethylation ( Genome[chr], IntronRight-125, IntronRight-26, WStrand, CStrand);
			} else if ( param.IR == "3SSR1" ) {
				if (IntronRight > IntronLeft + 25)
					RegionBiasedMethylation ( Genome[chr], IntronRight-25, IntronRight-5, WStrand, CStrand);
			} else { 
				cout << "[ERROR] Unknown parameter:\t" << param.IR << endl << "============" << endl;
				exit_with_help(); 
			}
		}
	//	cout << WStrand.size() << "\t" << CStrand.size() << endl;
		if ( WStrand.size() != 0 && CStrand.size() != 0 ) {
			...
			if (methylC >= SMALLNUM && methylW >= SMALLNUM ) {
				...
				float methylB = (methylW * WStrand.size() + methylC * CStrand.size()) 
					/ (WStrand.size() + CStrand.size() ); // Weighted average
				cout << liter->gene << "\t" << liter->trans << "\t" << log(AntiToSense)/log(2) << "\t" << methylB << endl;
			}
		//	cout << liter->gene << "\t" << liter->trans << "\t" << methylW << "\t" << methylC << endl;
		} 
	}	
...
//	WriteRefFlatFile ( Annotation );
	...
	IntronBiasedMethylation ( Genome, Annotation );
