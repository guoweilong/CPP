...
{
	printf(
		"Usage:	BiasMethyl -s <site.bed> -r <refFlat>\n"
		"Author:	Guo, Weilong	guoweilong@gmail.com 	2011-10-23\n"
		"Options:\n"
...
float MethylMean ( list<Site> & mC ) {
	...
	for ( list<Site>::iterator iter = mC.begin(); iter !=mC.end(); iter++, count++ ) 
		sum += iter->ML;
	return sum / count + 0.0001; // To return a non-zero value.
}
	...
	for ( liter = Annotation.begin(); liter != Annotation.end(); liter++ ) {
		...
		if ( WStrand.size() == 0 || CStrand.size() == 0 ) {
			...
			float methylB = (methylW * WStrand.size() + methylC * CStrand.size()) 
				/ (WStrand.size() + CStrand.size() ); // Both strands
			cout << liter->gene << "\t" << liter->trans << "\t" << AntiToSense << "\t" << methylB << endl;
		}
	}	
