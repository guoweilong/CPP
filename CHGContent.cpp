...
{
	printf(
		"Usage: CHGContent -c <int> -f <input.fa> [-s]\n"
		"Author: GUO Weilong	Version:2011/06/12\n"
		"options:\n"
		"-f	Input FASTA file\n"
...
int GetContent (vector<seqline> &Seq) {
	...
	unsigned int *CAG = new unsigned int[len - 4]; 
	unsigned int *CCG = new unsigned int[len - 4];
	unsigned int *CTG = new unsigned int[len - 4]; 
	...
	for (int i = 0; i < len - 4; i++) {
		cout << i - param.center << "\t" << CAG[i] << "\t" << CCG[i] << "\t" << CTG[i] << endl;
	}
	return 0;
}
