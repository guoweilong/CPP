...
{
	printf(
		"Usage:	BiasMethylNew -s <site.bed> -r <refFlat> \n"
		"Author:	Guo, Weilong	guoweilong@gmail.com 	2011-12-05\n"
		"Options:\n"
...
int IntronBiasedMethylation ( map< string, vector<Site> > & Genome, list<RefFlat> & Annotation ) {
	...
	for ( liter = Annotation.begin(); liter != Annotation.end(); liter++ ) {
		...
		if ( WStrand.size() != 0 && CStrand.size() != 0 ) {
			...
			if ( strand == '+' ) {
				AntiToSense = (methylC + SMALLNUM) / (methylW + SMALLNUM);
			} else {
				AntiToSense = (methylW + SMALLNUM) / (methylC + SMALLNUM);
			}
